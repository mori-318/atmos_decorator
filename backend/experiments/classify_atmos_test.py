...
class ClassifyAtmos:
    ...
    def run_classify(self, img_path):
        # モデルによる雰囲気分類
        label_by_model, ratio_by_model = self.classify_by_model(img_path)
        ...
        # 顔が検出された場合
        if is_face:
            label_by_smile, ratio_by_smile = self.classify_face_smiles(cv2.imread(img_path), faces)
...
if __name__ == "__main__":
    ...
    print("==== negative ===")
    classify_images("negative_test")
    print("finish\n")

    print("==== positive ===")
    classify_images("positive_test")
    print("finish\n")